...
| 이벤트 | 설명 |
| :--- | :--- |
| PlaybackNearlyFinished | 오디오 파일 재생이 끝나기 전에 전달되며, 다음의 두 가지 처리 방안이 있습니다.   1. 플레이 리스트를 재생하는 경우에는 다음 곡을 이어서 재생하기 위한 Play Directive를 전송합니다.   2. 더 이상 재생할 곡이 없으면 Stop Directive를 전송합니다.\(필수 사항은 아니지만 Stop Directive 전송을 권장합니다.\) |
